...
//  This is the command-line version of this program; it has been cleaned up
//  from Martin Guy's original code, just to compile cleanly, with
//  warnings turned on, using Gnu g++ 3.4.5
//  It should still be fairly portable.
//  
//  I also switched from using redirection to read the dictionary, 
//  to passing as command-line argument.
//  This is preparation for doing a Windows version of the program.
//  
//  compile: g++ -Wall -O2 -s anagram.cpp -o anagram.exe 
//       or: make anagram
//***************************************************************************
/*
...
static uint min_word_len = 3;

static char word_list_name[PATH_MAX + 1] = "words";

//  function prototypes
cell_t *buildwordlist (void);
int sameletters (char *word1, char *word2);
static int islonger (char *first, char *second);
cell_t *sort (cell_t * head);
void oom (char *where);
void initfind (cell_t * head);
void findanags (int generation, cell_t * wordlt, int nleft);
cell_t *forgelinks (cell_t * head);
void print (int gen, int higen);
char *savestr (char *oldstr);
void printwordlist (cell_t * head);

//****************************************************************************
static void usage (void)
{
   fprintf (stderr, "Usage: %s -wword_list_name -v -m# word [word] ...\n", progname);
   fprintf (stderr, "-w\tname of word file (default = ./words )\n");
...
int main (int argc, char **argv)
{
      ...
      if (*p == '-') {
         p++ ; // skip the switch char
            ...
            case 'v':        /* verbose mode */
               vflag = 1;
               break;
            case 'w':        //  read alternate word list
               p++;          //  skip the 'w'
               strncpy (word_list_name, p, PATH_MAX);
   ...
   if (vflag)
		fprintf (stderr, "%d letters in key.\n", nletters);
	/*
	...
	if (vflag)
		fputs ("Building wordlist...\n", stderr);
	wordlist = buildwordlist ();
	if (wordlist == NULL) {
		if (vflag)
			fputs ("Empty dictionary or no suitable words.\n", stderr);
		exit (0);
	}

	if (vflag)
		fputs ("Sorting wordlist...\n", stderr);
...
cell_t *buildwordlist (void)
{
	...
	if (vflag)
		fprintf (stderr, "%d suitable words found\n", nwords);
	fclose (fd);
	return (head);
}
...
cell_t *sort (cell_t * head)
{
	...
	if (sortbase == NULL)
		oom ("buildwordlist while sorting");
	/*
...
char *savestr (char *oldstr)
{
	...
	newstr = (char *) malloc ((unsigned) (cp - oldstr + 1));	/* +1 for the null */
	if (newstr == NULL)
		oom ("savestr");
